...
class SerializationTool(object):
    ...
    @staticmethod
    def serialize_model(model: torch.nn.Module, use_peft: bool = True, cpu: bool = True) -> torch.Tensor:
        ...
        Args:
            ...
            use_peft (bool, optional): Whether to include the parameters in the PEFT module. Defaults to ``True``.
            cpu (bool, optional): Whether move the vectorized parameter to ``torch.device('cpu')`` by force. Defaults to ``True``. If ``cpu`` is ``False``, the returned vector is on the same device as ``model``.
        """
        if use_peft:
            parameters = [param.data.view(-1) for param in get_peft_model_state_dict(model).values()]
        else:
    ...
    def deserialize_model(model: torch.nn.Module,
        ...
        if use_peft:
            for param in get_peft_model_state_dict(model).values():
                numel = param.numel()
                size = param.size()
                if mode == "copy":
                    param.copy_(
