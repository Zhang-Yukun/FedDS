...
def main(args):
    ...
    for rd in range(0, config["n_round"] + 1):
        ...
        if rd == 0:
            ...
            model.from_pretrained(config["full_model_path"])
            logger.info(f'*** Round {rd} ** New Model initialized!')
            if "lora_args" in config:
                peft_config = LoraConfig(**config["lora_args"])
                peft_model = get_peft_model(model, peft_config)
                logger.info(f'*** Round {rd} ** New Model initialized with PEFT!')
            ...
            logger.info(f'*** Round {rd} ** Model loaded!')
